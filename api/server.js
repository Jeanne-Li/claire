...
async function saveArticlesToDatabase(articles) {
  ...
  try {
      ...
      for (const articleData of articles) {
          ...
          if (id) {
              logger.info(`Article saved to the database: ${utf8Title} with PDF stored in the database.`);
          } else {
            ...
            try {
                ...
                if (articleData.title !== 'Title not found') {
                    ...
                    articlesBatch.push({
                        ...articleData,
                        url: articleUrl,
                        filePath: path.basename(finalFilePath)
                    });
...
app.get('/pdf/:id', async (req, res) => {
    const client = await pool.connect();
    try {
        const result = await client.query('SELECT pdf_url FROM scraper_presse_citron WHERE id = $1', [req.params.id]);
        if (result.rows.length > 0) {
            const pdfBinaryData = result.rows[0].pdf_url;
            if (pdfBinaryData) {
